...
def main():
    ...
    try:
        ...
        check("objects 3 个", set(ob) == {"cola_can_1", "bottled_water_1", "mobile_phone_1"})
        ...
        try:
            ...
            def nav_side2():
                # 等 /nav 写出第二个任务目录后写回结果
                deadline = time.time() + 4
                while time.time() < deadline:
                    dirs = [d for d in os.listdir(os.path.join(tmp, "tasks"))
                            if d.startswith("fq_brain_nav") and d not in os.path.basename(task_dir)]
                    for d in dirs:
                        tp = os.path.join(tmp, "tasks", d, "navigation_task.yaml")
                        rp = os.path.join(tmp, "tasks", d, "navigation_result.yaml")
                        if os.path.isfile(tp) and not os.path.isfile(rp):
                            with open(tp) as f:
                                t = yaml.safe_load(f)
                            with open(rp, "w") as f:
                                yaml.safe_dump({"task_id": t["task"]["task_id"], "revision": 1,
                                                "status": "SUCCEEDED",
                                                "final_pose_xyt_rad": t["goal"]["pose_xyt_rad"]}, f)
                            return
                    time.sleep(0.1)

            threading.Thread(target=nav_side2, daemon=True).start()
            r = c.post("/nav", json={"x": 0.95, "y": -1.70, "target_yaw": -134.6}).get_json()
            check("POST /nav 全链(写任务→收结果)", r["success"] is True, r["result"])
        finally:
            with open(cfg_path, "w", encoding="utf-8") as f:
                f.write(cfg_backup)
