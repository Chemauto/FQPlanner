...
智能工具匹配模块，根据任务描述自动选择最相关的工具。

## 特性

- **多权重评分系统**：结合语义相似度、关键词匹配和分类相关性进行综合评分
- **智能降级机制**：当某些组件不可用时（如网络问题、缺少依赖），自动回退到其他评分方式
- **LRU 缓存优化**：使用最近最少使用缓存机制优化查询性能
- **分类管理**：支持工具分类和按分类搜索
- **灵活配置**：可配置最大工具数量、最低相似度阈值等参数

## 核心组件

### ToolMatcher
智能工具匹配器，负责计算工具与任务之间的匹配分数。

**主要功能：**
- 语义相似度计算（基于 sentence-transformers）
- 关键词匹配评分
- 分类相关性评分
- 多权重综合评分
- 降级机制管理
...
### 权重配置
- **语义相似度**：70%（需要 sentence-transformers）
- **关键词匹配**：20%
- **分类相关性**：10%
...
- `get_tools_by_category(category)`：按分类获取工具
- `get_stats()`：获取统计信息
...
1. 首次使用时 sentence-transformers 模型会自动下载
2. 网络不可用时系统自动降级为关键词匹配
3. 建议工具描述使用英文关键词以获得更好的匹配效果
4. 缓存机制自动管理内存，无需手动清理
